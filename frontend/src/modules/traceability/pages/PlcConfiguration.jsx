...
import { Cpu, Plus, Trash2, RefreshCw, Activity } from "lucide-react";
...
const PlcConfiguration = () => {
  ...
  const handleTestRead = async (range) => {
    setTestingId(range.id);
    setReadResult(null);
    try {
      const payload = {
        count: Math.min(Number(range.rangeSize || 10), 10),
        device: "R",
        frameMode: "AUTO",
        timeoutMs: 5000,
      };
      const data = await plcConfigApi.testReadRange(range.id, payload, { timeout: 30000 });
      setReadResult(data);
      const values = data?.read?.values || {};
      const errors = data?.read?.errors || [];
      const valueCount = Object.keys(values).length;
      if (valueCount > 0) toast.success(`Read ${valueCount} register${valueCount === 1 ? "" : "s"}`);
      else if (errors.length) toast.error(errors[0]?.message || "PLC read failed");
      else toast.success("PLC endpoint reachable");
    } catch (err) {
      const message = err.response?.data?.error || "PLC test/read failed";
      setReadResult({ ok: false, range, error: message });
      toast.error(message);
    } finally {
      setTestingId(null);
    }
  ...
  return (
    <div className="space-y-6 rise-in">
                  ...
                  <tr key={r.id} className="hover:bg-bg-dark/20 transition-colors group">
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => handleTestRead(r)}
                          disabled={testingId === r.id}
                          className="inline-flex items-center gap-1.5 px-3 py-2 text-[11px] font-bold text-text-muted hover:text-primary hover:bg-primary/10 rounded-lg transition-all disabled:opacity-50"
                          title="Test and read first registers"
                        >
                          <Activity size={13} />
                          {testingId === r.id ? "Reading..." : "Test/Read"}
                        </button>
      ...
      {readResult && (
        <div className="bg-bg-card border border-border rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex justify-between bg-bg-dark/30 items-center">
            <h2 className="text-xs font-bold text-text-main uppercase tracking-wider">PLC Test Result</h2>
            <span className={`text-[11px] px-3 py-1 rounded-lg border ${readResult.ok ? "text-emerald-400 border-emerald-500/30 bg-emerald-500/10" : "text-danger border-danger/30 bg-danger/10"}`}>
              {readResult.ok ? "Connected" : "Failed"}
            </span>
          </div>
          <div className="p-5 space-y-4">
            {readResult.error ? (
              <p className="text-sm text-danger font-mono break-all">{readResult.error}</p>
            ) : (
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                  <div className="bg-bg-dark/40 border border-border rounded-xl p-3">
                    <p className="text-[10px] uppercase tracking-wider text-text-muted font-bold">Endpoint</p>
                    <p className="font-mono text-text-main mt-1">{readResult.endpoint?.ip}:{readResult.endpoint?.port}</p>
                  </div>
                  <div className="bg-bg-dark/40 border border-border rounded-xl p-3">
                    <p className="text-[10px] uppercase tracking-wider text-text-muted font-bold">Protocol</p>
                    <p className="font-mono text-text-main mt-1">{readResult.endpoint?.protocol}</p>
                  </div>
                  <div className="bg-bg-dark/40 border border-border rounded-xl p-3">
                    <p className="text-[10px] uppercase tracking-wider text-text-muted font-bold">Device</p>
                    <p className="font-mono text-text-main mt-1">{readResult.read?.device || "Holding"}</p>
                  </div>
                </div>
                {readResult.read?.message && <p className="text-sm text-text-muted">{readResult.read.message}</p>}
                {Object.keys(readResult.read?.values || {}).length > 0 && (
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                    {Object.entries(readResult.read.values).map(([register, value]) => (
                      <div key={register} className="bg-bg-dark border border-border rounded-xl px-3 py-2">
                        <p className="text-[10px] text-text-muted font-mono">R{register}</p>
                        <p className="text-lg font-black text-text-main font-mono">{String(value)}</p>
                      </div>
                    ))}
                  </div>
                )}
                {(readResult.read?.errors || []).length > 0 && (
                  <div className="space-y-2">
                    {readResult.read.errors.slice(0, 3).map((error, idx) => (
                      <p key={idx} className="text-xs text-danger font-mono break-all">
                        R{error.register}: {error.message}
                      </p>
                    ))}
                  </div>
                )}
