...
function buildSlmpAsciiReadData({ address, device, count }) {
...
function buildSlmpAsciiWriteData({ address, device, values }) {
  const words = (Array.isArray(values) ? values : [])
    .map((value) => toUInt16(value).toString(16).toUpperCase().padStart(4, "0"))
    .join("");
  return `${buildSlmpAsciiReadData({ address, device, count: values.length })}${words}`;
}
...
async function readSlmpWords(socket, { device, address, count, timeoutMs, route }) {
  ...
  const frame =
    frameMode === "ASCII"
      ? buildSlmpAsciiFrame({
          command: 0x0401,
          subcommand: 0x0000,
          asciiData: buildSlmpAsciiReadData({ address, device, count }),
          ...route,
        })
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    if (offset + 2 <= payload.length) {
      values.push(frameMode === "ASCII" ? payload.readUInt16BE(offset) : payload.readUInt16LE(offset));
    }
  }
  return values;
}
  ...
  const frame =
    frameMode === "ASCII"
      ? buildSlmpAsciiFrame({
          command: 0x1401,
          subcommand: 0x0000,
          asciiData: buildSlmpAsciiWriteData({ address, device, values }),
          ...route,
        })
