...
function buildAsciiReadData({ address, device, count }) {
  const deviceCode = ASCII_DEVICE_CODES[device] || ASCII_DEVICE_CODES.D;
  const deviceNo = Math.max(0, Number(address) || 0).toString().padStart(6, "0");
  const pointCount = toUInt16(count).toString(16).toUpperCase().padStart(4, "0");
  return `${deviceCode}${deviceNo}${pointCount}`;
}

function buildAsciiWriteData({ address, device, values }) {
  const words = (Array.isArray(values) ? values : [])
    .map((value) => toUInt16(value).toString(16).toUpperCase().padStart(4, "0"))
    .join("");
  return `${buildAsciiReadData({ address, device, count: values.length })}${words}`;
}
...
function buildFrameAscii({ command, subcommand, data = Buffer.alloc(0), asciiData = null, monitoringTimer = 0x0010 }) {
  const networkNo = toByte(process.env.PLC_SLMP_NETWORK_NO || 0);
  const plcNo = toByte(process.env.PLC_SLMP_PLC_NO || 0xff);
  const ioNo = toUInt16(process.env.PLC_SLMP_IO_NO || 0x03ff);
  const stationNo = toByte(process.env.PLC_SLMP_STATION_NO || 0);
  ...
  const payloadHex =
    `${toHexUInt16(monitoringTimer)}` +
    `${toHexUInt16(command)}` +
    `${toHexUInt16(subcommand)}` +
    `${asciiData !== null ? String(asciiData).toUpperCase() : data.toString("hex").toUpperCase()}`;
...
async function readWords(socket, { device, address, count, timeoutMs, frameMode = "BINARY" }) {
  ...
  const frame = buildFrame(
    {
      ...
      subcommand: 0x0000,
      data,
      asciiData: buildAsciiReadData({ address, device, count }),
    },
    frameMode
  );
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    if (offset + 2 <= payload.length) {
      values.push(normalizeFrameMode(frameMode, "BINARY") === "ASCII" ? payload.readUInt16BE(offset) : payload.readUInt16LE(offset));
    }
  }
  return values;
}
  ...
  const frame = buildFrame(
    {
      ...
      subcommand: 0x0000,
      data,
      asciiData: buildAsciiWriteData({ address, device, values }),
    },
    frameMode
  );
  const packet = await sendAndReceivePacket(socket, frame, timeoutMs, frameMode);
