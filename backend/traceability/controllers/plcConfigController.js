...
function normalizeSlmpDevice(value, fallback = "R") {
  const device = String(value || "").trim().toUpperCase();
  return ["D", "M", "X", "Y", "W", "L", "F", "V", "B", "R"].includes(device) ? device : fallback;
}
...
exports.testRangeRead = async (req, res) => {
  try {
    const id = toInt(req.params.id);
    if (!id) {
      return res.status(400).json({ error: "Invalid range id" });
    ...
    if (!range.plc_ip || !range.plc_port) {
      return res.status(400).json({ error: "PLC IP and port are required for this register block" });
    }

    const timeoutMs = Math.min(Math.max(toInt(req.body?.timeoutMs ?? req.query?.timeoutMs) || 5000, 300), 30000);
    const maxCount = Math.min(Math.max(toInt(req.body?.count ?? req.query?.count) || range.range_size, 1), 100);
    const registers = [];
    for (let reg = range.range_start; reg <= range.range_end && registers.length < maxCount; reg += 1) {
      registers.push(reg);
    }

    let read = null;
    if (range.plc_protocol === "MODBUS_TCP") {
      read = await readModbusRegisters({
        ip: range.plc_ip,
        port: range.plc_port,
        unitId: toInt(req.body?.unitId ?? req.query?.unitId) || 1,
        registers,
        timeoutMs,
      });
    } else if (range.plc_protocol === "SLMP") {
      const device = normalizeSlmpDevice(req.body?.device ?? req.query?.device, "R");
      const frameMode = String(req.body?.frameMode ?? req.query?.frameMode ?? "AUTO").trim().toUpperCase();
      read = await readSlmpRegisters({
        ip: range.plc_ip,
        port: range.plc_port,
        registers: registers.map((register) => ({ register, device })),
        defaultDevice: device,
        frameMode,
        timeoutMs,
      });
      read.device = device;
      read.frameMode = frameMode;
    } else {
      await probeTcpEndpoint({ ip: range.plc_ip, port: range.plc_port, timeoutMs });
      read = { values: {}, errors: [], message: "TCP endpoint is reachable. Register reads require MODBUS_TCP or SLMP." };
    }
