...
const { ensureIotSchema } = require("./scripts/ensureIotSchema");
...
const io = new Server(server, {
  ...
  pingTimeout: 60000,        // wait 60s before declaring client dead
  pingInterval: 25000,       // send ping every 25s
  connectTimeout: 45000,     // allow 45s for initial handshake
...
app.get("/", (_req, res) => {
  res.json({
    message: "Unified Traceability + Rico IoT Backend Running",
    database: "mssql",
  });
});
...
io.on("connection", (socket) => {
  console.log(`[Socket] Client connected    | id: ${socket.id} | transport: ${socket.conn.transport.name}`);

  // ── LOG TRANSPORT UPGRADES (polling → websocket) ──────────────
  socket.conn.on("upgrade", (transport) => {
    console.log(`[Socket] Transport upgraded  | id: ${socket.id} | new transport: ${transport.name}`);
  });
  ...
  socket.on("subscribe_part", (payload = {}) => {
    ...
    if (room) {
      socket.join(room);
      console.log(`[Socket] Subscribed to part   | id: ${socket.id} | partId: ${partId}`);
    }
  });
  ...
  socket.on("unsubscribe_part", (payload = {}) => {
    ...
    if (room) {
      socket.leave(room);
      console.log(`[Socket] Unsubscribed part    | id: ${socket.id} | partId: ${partId}`);
    }
  });

  // ── DISCONNECT WITH REASON ────────────────────────────────────
  // Common reasons:
  //   "transport close"      → network dropped / browser tab closed
  //   "transport error"      → connection error (CORS, proxy, etc.)
  ...
  socket.on("connect_error", (err) => {
    console.error(`[Socket] Connection error    | id: ${socket.id} | message: ${err.message}`);
  });
});
...
// Repeated alter can create unnecessary SQL Server schema/index changes.
...
async function startServer() {
  try {
    console.log("[Startup] authenticate...");
    await sequelize.authenticate();
    console.log("[Startup] authenticate complete");
    console.log("[Startup] sequelize sync...");
    await sequelize.sync({ alter: syncAlter, force: syncForce });
    console.log("[Startup] sequelize sync complete");
    await runStartupDbTask("ensureIotSchema", () => ensureIotSchema());
