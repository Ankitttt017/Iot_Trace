...
const getMachines = async (req, res) => {
  try {
    ...
    const params = [];
    const where = plant ? "WHERE m.plant_code = ?" : "";
    ...
    const { rows } = await db.query(`
      SELECT
        ...
        COALESCE(primary_operation.part_code, ms.part_code) AS part_code,
        COALESCE(p.description, primary_operation.part_code, ms.part_code, 'No part assigned') AS part,
        COALESCE(primary_operation.operation_no, ms.operation_no) AS operation_no,
        COALESCE(operation_counts.assigned_operation_count, 0) AS assigned_operation_count,
      ...
      OUTER APPLY (
        SELECT TOP 1 mo.part_code, mo.operation_no, mo.updated_at
        FROM ${TABLES.machineOperations} mo
        WHERE mo.machine_id = m.id AND mo.is_active = 1
        ORDER BY mo.is_primary DESC, mo.updated_at DESC, mo.id DESC
      ) primary_operation
      LEFT JOIN (
        ...
        WHERE is_active = 1
        GROUP BY machine_id
      ) operation_counts ON operation_counts.machine_id = m.id
      LEFT JOIN ${TABLES.parts} p ON p.material_code = COALESCE(primary_operation.part_code, ms.part_code)
      ${where}
      ORDER BY name ASC
    `, params);
